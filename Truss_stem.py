...
class TrussController():
    ...
    def ImportFromFile(self, data):
        ...
        title, material, static_factor, nodes, links = None, None, None, [], []
        ...
        with open(filename, 'r') as file:
            for line in file:
                ...
                # Split the line by comma and strip whitespace from each element
                parts = [part.strip() for part in line.split(',')]

                # Parse the line based on the first element (keyword)
                if parts[0].lower() == 'title':
                    title = parts[1].strip("'")
                elif parts[0].lower() == 'material':
                    material = Material(*map(float, parts[1:]))
                elif parts[0].lower() == 'static_factor':
                    static_factor = float(parts[1])
                elif parts[0].lower() == 'node':
                    nodes.append(Node(parts[1], Position((float(parts[2]), float(parts[3]), 0))))
                elif parts[0].lower() == 'link':
                    links.append((parts[1], parts[2], parts[3]))

        # Assign parsed data to the truss model
        self.truss.title = title
        self.truss.material = material
        self.truss.material.staticFactor = static_factor
        for node in nodes:
            self.addNode(node)
        for link in links:
            node1 = self.getNode(link[1])
            node2 = self.getNode(link[2])
            if node1 and node2:
                self.addLink(Link(name=link[0], node1=link[1], node2=link[2]))

        # Calculate link values, update the display, and draw the truss
        self.calcLinkVals()
        self.displayReport()
        self.drawTruss()
